...
def _normalize_group(group: dict[str, dict[str, int]]) -> dict[str, dict[str, Any]]:
...
def _usage_summary(
    ...
    tiers = _normalize_group(by_tier)
    stages = _normalize_group(by_stage)
...
def usage_delta(current: dict[str, Any], previous: dict[str, Any]) -> dict[str, Any]:
    tier_delta = _group_delta(current["by_tier"], previous["by_tier"])
    stage_delta = _group_delta(current["by_stage"], previous["by_stage"])
...
def merge_usage_summaries(accumulated: dict[str, Any], increment: dict[str, Any]) -> dict[str, Any]:
    tiers = _merge_groups(accumulated["by_tier"], increment["by_tier"])
    stages = _merge_groups(accumulated["by_stage"], increment["by_stage"])
    return _usage_summary(tiers, stages)


class UsageTracker:
    """线程安全地累加标准化用量，按 tier 与调用 stage 分别归因。"""
