...
状态：已完成（320201a / 13240bd / 9a58459 / 5bacc1d）
...
- 测试：全量 `uv run pytest -q` **333 passed**（323 基线 + #4 四个 + #3 三个 + #5 三个）；
  `ruff check .` + `ruff format --check .` 全绿；全程离线（MockTransport + pymupdf 生成 PDF）。
- 提交（每批一提交，均已双推 GitHub/Gitee main）：
  - C1 `320201a` docs: 立项 issue 清理计划（#3–#6）
  - C2 `13240bd` fix(glossary): 术语命中 target/candidates NFKC 归一化（#4）
  - C3 `9a58459` fix(g0): 句末注码排除紧跟中文/百分号数字（#3）
  - C4 `5bacc1d` feat(mineru): >200 页自动分批（#5）
- issue 处置（维护者身份回帖 + 关闭）：
  - #4 → comment `5652014916`（修复提交 + 改动说明）
  - #3 → comment `5652021865`（口径：4 位年份在 main 不复现，实际修复类为
    1–3 位数字紧跟中文/%；残余边界已注明）
  - #5 → comment `5652037542`（顺序分批实现方式、合并保证、本地分支可弃）
  - #6 → comment `5652039463`（三条建议逐条映射既有提交 + content_desc 维持
    W 级理由 + lesson 指引）
- lesson 评估：两处 g0 缺陷为纯函数对称性/排除集修复，经验本体已固化在
  `g0.py` / `terms.py` 注释与回归测试，**不新增 lesson**（避免与代码注释重复）。

## 实施偏差

- #5 图片防撞的实现位置：按计划在「合并层」做（`images` key 与 `img_path` 同步加
  `b{批号}/` 前缀），而非 issue 建议的「media 落盘文件名加批次前缀」——落盘名由
  `next_insert_id` 按**全局页**生成（跨批天然唯一），改落盘名反而破坏
  `raw/inserts/<id>.json` 的可追溯命名约定。
- #3 实际修复范围小于 issue 标题描述：4 位年份样例在当前 main 不复现
  （`(?!\d)` 回溯已排除），修复聚焦实测误报类（1–3 位数字紧跟中文/百分号）；
  回帖已向协作者说明。
- #5 `_parse_maybe_batched` 的每批页数由「总页数 + 阈值」推导，未逐批重开 PDF
  读页数（等价且省一次 IO；与切页逻辑共享同一 range 边界）。
