> **中文** | [English](development-plan.md)

# auto-epublizer 项目开发计划

> ⚠️ **历史规划（已基本完成）**：其中的 LLM 抽象模块（`llm/`、`agents/`、`analyze`/
> `translate`/`review` 命令）已按**唯一 LLM 原则**移除（CLI 零 LLM 调用，语义工作由
> 操作 CLI 的 agent 完成）。本文件保留作为架构演进的原始设计记录。
