...
- **本项目不审查处理对象的版权风险**：源文件等权属由用户负责；工作区（含 `source/`）
  **全量纳入 git 仓库**，默认作为**私有仓库**（托管平台可配置，默认 GitHub）；
  转公开需用户显式声明或自行处理。
- **权属不清 → 默认私有仓库分发，或停下询问用户**；不要替用户做法律假设。
- 权属结论记录进发布说明（一句话+依据），不写进代码或配置。
- **统一术语库/知识库**（工作区之外，默认 `~/Documents/auto-epublizer`）同为**私有 git
  仓库**，可推送托管平台跨设备；其内容同样由用户负责，转公开前须完成本节隐私扫描。
...
| 原书正文版权材料 | 源文件（`source/`）随工作区**全量入库**（默认私有仓库；版权风险由用户负责，本项目不审查） | 转公开前必须先完成本表扫描，并由用户显式声明 |
...
- **工作区全量入库（含 `source/`）且默认私有**：仓库默认**私有**（托管平台可配置，
  默认 GitHub）；转公开前必须完成 §1.2 隐私扫描，并由用户显式声明。
